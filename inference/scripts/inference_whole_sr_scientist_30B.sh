...
OUTPUT_JSON_PATH="./output/memory_default/qwen3-coder-30b_tool2_0_001_s40_max_25_top3_N1.json"
MODEL_PATH="../models/SR-Scientist-30B"
TOOL_SCHEMA="tool2"
...
echo "--- EXECUTING VLLM COMMAND ---" >> "$LOG_FILE"
...
echo "Waiting for VLLM server to be fully ready by sending test requests to ${CHAT_API_URL}..."
...
while true; do
    ...
    if [ "$STATUS_CODE" -eq 200 ]; then
        echo "VLLM server is responsive and ready for inference! API Status: 200."
        break
    else
        echo "VLLM server not fully ready (API Status: $STATUS_CODE). Retrying in 15 seconds..."
        sleep 15
    fi
done
