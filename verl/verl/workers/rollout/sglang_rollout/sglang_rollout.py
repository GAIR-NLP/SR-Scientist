...
class SGLangRollout(BaseRollout):
    ...
    def _initialize_tools(self, config, processing_class):
        ...
        if config.multi_turn.tool_call_parser:
            tool_call_parser_type = config.multi_turn.tool_call_parser
        else:
            tool_call_parser_type = get_tool_call_parser_type(processing_class)
        print(f"tool call parser type {tool_call_parser_type}")
        ...
        while current_turns < self.config.multi_turn.max_assistant_turns:
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                ...
                if _req.messages[-1].tool_calls is not None:
                    ...
                    # print(f"tool call result: {tool_call_results}")
    ...
    def _req_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        # print(f"Rank {self._rank} TP Rank {self._tp_rank} {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        if self._tp_rank == 0:
